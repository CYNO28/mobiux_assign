...
2019-03-31,Butterscotch Single Scoop,60,2,120`;
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      obj.date = str[i][0];
      obj.name = str[i][1];
      obj.unit_price = str[i][2];
      obj.quantity = str[i][3];
      obj.price = str[i][4];
      arr.push(obj);
    }
    ...
    for (let i = 0; i < arr.length; i++) {
      total_price += Number(arr[i].price);
    }
    console.log("Total of Price is :", total_price);

    // month wise sale

    let obj = {};
    for (let i = 0; i < arr.length; i++) {
      let month = arr[i].date.split("-")[1];
      if (!obj[month]) {
        obj[month] = Number(arr[i].price);
      } else {
        obj[month] += Number(arr[i].price);
      }
    }
    ...
    for (key in obj) {
      console.log("sale in month ", months[Number(key) - 1], "is", obj[key]);
    }
    let month_wise_sum={...obj}
    
    // monthwise popular item
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (!obj[month]) {
        obj[month] = [];
        obj[month].push(arr[i]);
      } else {
        obj[month].push(arr[i]);
      }
    }
    let month_wise_data={...obj}
    for (key in obj) {
      let month_data = obj[key];
      let sub_obj = {};
      for (let i = 0; i < month_data.length; i++) {
        if (!sub_obj[month_data[i].name]) {
          sub_obj[month_data[i].name] = Number(month_data[i].quantity);
        } else {
          sub_obj[month_data[i].name] += Number(month_data[i].quantity);
        }
      }
      ...
      month_wise_pop.push(product_name)
      console.log(
        "most sold product in month ",
        months[Number(key) - 1],
        "is :",
        product_name,
    ...
    for (key in obj) {
        ...
        for (let i = 0; i < month_data.length; i++) {
          if (!sub_obj[month_data[i].name]) {
            sub_obj[month_data[i].name] = Number(month_data[i].price);
          } else {
            sub_obj[month_data[i].name] += Number(month_data[i].price);
          }
        }
    ...
    // this is per order  min,max and avg not day wise
    for(key in month_wise_data){
        let min_sale =Infinity
        let max_sale=-Infinity
        let sum=0   
        let count=0  //need sum and count for avg
        let pop_product=month_wise_pop[Number(key)-1]
        let month_data = obj[key];
        for(let i = 0; i < month_data.length; i++){
            if(month_data[i].name==pop_product){
               
                sum+=Number(month_data[i].quantity)
                if(max_sale<Number(month_data[i].quantity)){
                   max_sale= Number(month_data[i].quantity)
                }
                if(min_sale>Number(month_data[i].quantity)){
                    min_sale=Number(month_data[i].quantity)
                }
                count++
            }
        }
        console.log("Month :",months[Number(key) - 1],"product :",pop_product,'max sale :',max_sale,'min sale :',min_sale,'avg :',sum/count)
       
    }
    
    
    
    //--extra insight how i am changing data 
    // console.log(month_wise_sum)
    // console.log(month_wise_data)
    
    
    
    // i am doing everthing seprate to make it easy to read 
    // use node index.js in terminal to run
